	...
	public class FinDePartie extends JFrame {
		...
		public void addComponentsToPane(Container pane, int[] score, int[] couleur, String[] nom,
				ArrayList<Integer> couleurs, int[] scoreCouronnes) {
			...
			System.out.println("Winner incontesté :" + (int) getWinner(score, scoreCouronnes));
			if (NB == 2) {
	
				first = getWinner(score, scoreCouronnes);
				if (first == 0) {
					second = 1;
				}
		...
		public FinDePartie(Parametres parametres, ArrayList<Integer> couleurs, int[] score, long Time,
				ArrayList<String> noms2, int[] scoreCouronnes) {
			...
			addComponentsToPane(this, score, couleur, nom, couleurs, scoreCouronnes);
		...
		private int getWinner(int[] score, int[] couronnes) {
			...
			for (int i = 0; i < this.nbTotal; i++) {
				if (score[i] > Winner) {
					Winner = score[i];
					WinnerCrowns = couronnes[i];
					out = i;
				}
				if (score[i] == Winner) {
	
					if (couronnes[i] > WinnerCrowns) {
						Winner = score[i];
						WinnerCrowns = couronnes[i];
						out = i;
					}
